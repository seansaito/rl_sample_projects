...
from environments import game_environ
env = game_environ(partial=False, size=5)
...
def update_target_aux(op_holder, sess):
    for op in op_holder:
        sess.run(op)
...
class QNetwork():
    def __init__(self, h_size):
        ...
        self.game_image = tf.reshape(self.input, shape=[-1, 84, 84, 3])
        self.conv1 = slim.conv2d(inputs=self.game_image, num_outputs=32, kernel_size=[8, 8],
...
if __name__ == '__main__':
    ...
    targetOps = update_target(train_variables, tau)

    game_buffer = experience_buffer()
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_episodes):
            ...
            while j < max_epLength:
                ...
                if total_steps > pre_train_steps:
                    ...
                    if total_steps % (update_freq) == 0:
                        trainBatch = game_buffer.sample(batch_size)
                        ...
                        update_target_aux(targetOps, sess)
                rAll += r
                s = s1
            ...
            game_buffer.add(episodeBuffer.buffer)
            jList.append(j)
            rList.append(rAll)
